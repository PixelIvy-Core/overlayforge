...
def setTrackInfo(id: str, title: str, artist: str, source: str):
    track = {"id": id, "title": title, "artist": artist, "source": source}
    with get_redis() as r:
        if not r:
    ...
    with get_redis() as r:
        ...
        if next_id:
            ...
            track = {"id":metadata["id"],
                    "url":get_gonic_url("stream",metadata["id"]),
                    "title":str.replace(metadata['title'], ".mp3", ""),
...
async def get_track_info(curr_id: str) -> AsyncIterable[TrackInfo]:
    ...
    if not track:
        raise HTTPException(404, "Track info not found")
    else:
        if (track["id"] != curr_id):
            info ={
                "id": track["id"],
                "url": get_gonic_url("stream", track["id"]),
                "title": track["title"],
                "artist": track["artist"],
                "source": track["source"]
            }
